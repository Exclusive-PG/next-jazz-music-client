...
import { defaultAuthOptions } from "~/server/auth";
...
function handler(
  ...
  return NextAuth(request, context, {
    ...defaultAuthOptions,
    callbacks: {
      ...defaultAuthOptions.callbacks,
      signIn: async ({ user }) => {
        if (isCredentialsCallback) {
          if (user) {
            const sessionToken = generateSessionToken();
            const sessionExpiry = getExpiryDate(
              defaultAuthOptions.session.maxAge,
            );

            await defaultAuthOptions.adapter.createSession?.({
              sessionToken,
              userId: user.id,
